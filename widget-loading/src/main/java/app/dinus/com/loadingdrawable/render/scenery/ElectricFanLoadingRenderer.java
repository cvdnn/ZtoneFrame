...
//            reset();
    ...
    private static final int DEFAULT_TEXT_COLOR = 0XFFE18D0B;
    ...
    private float mAnimFrameValue;
    private float mTempProgressWidth = -1F;
...
//        mLoadingDrawable.setBounds(-mLoadingDrawable.getIntrinsicWidth() / 2,
//                0,
...
//        mLoadingDrawable.draw(canvas);
        ...
        if (!TextUtils.isEmpty(mText)) {
            ...
            canvas.drawText(mText, (bounds.width() - textRect.width()) / 2.0f, mLoadingDrawable.getIntrinsicHeight(), mPaint);
        }
...
//        createProgressPath(mProgress, mProgressCenterRadius, progressRect);

        //draw progress
        mPaint.setColor(mProgressColor);
        mPaint.setStyle(Paint.Style.FILL);
        Path path = progressPath(mProgress, mProgressCenterRadius, progressRect);
        ...
        float progress = mAnimFrameValue;
        float circleRadius = mProgressCenterRadius;
        float progressWidth = progress * progressRect.width();

        if (mProgress >= 1.0) {
            if (mTempProgressWidth < 0) {
                mTempProgressWidth = progressWidth;
            }

            mScale = 1 - (progressWidth - mTempProgressWidth) / circleRadius;
            if ((progressWidth - mTempProgressWidth) >= circleRadius) {
                stop();
            }
        }

        float rotateScale = mScale > 1f ? 1f : (mScale < 0f ? 0f : mScale);
        ...
        mElectricFanDrawable.setBounds((int) (electricFanCenterX - mElectricFanDrawable.getIntrinsicWidth() / 2 * rotateScale),
                (int) (electricFanCenterY - mElectricFanDrawable.getIntrinsicHeight() / 2 * rotateScale),
                (int) (electricFanCenterX + mElectricFanDrawable.getIntrinsicWidth() / 2 * rotateScale),
                (int) (electricFanCenterY + mElectricFanDrawable.getIntrinsicHeight() / 2 * rotateScale));
        ...
        //draw 100% text
        if (rotateScale < 1.0f) {
            mPaint.setTextSize(mTextSize * (1 - rotateScale));
            mPaint.setColor(mElectricFanOutlineColor);
            Rect textRect = new Rect();
    ...
    private Path progressPath(float progress, float circleRadius, RectF progressRect) {
        RectF arcProgressRect = new RectF(progressRect.left, progressRect.top, progressRect.left + circleRadius * 2, progressRect.bottom);
        RectF rectProgressRect = null;
        ...
        //the left of the right half circle
        if (progress > 0 && progressWidth < progressRect.width() - circleRadius) {
            RectF leafRect = new RectF(progressRect.left + progressWidth, progressRect.top, progressRect.right - circleRadius, progressRect.bottom);
            addLeaf(progress, leafRect);
        }
    ...
    private void scaleRotation(float progress, float circleRadius, RectF progressRect) {
        float progressWidth = progress * progressRect.width();
    ...
    public void computeRender(float renderProgress) {
        if (renderProgress < DECELERATE_DURATION_PERCENTAGE) {
            mAnimFrameValue = LINEAR_INTERPOLATOR.getInterpolation(renderProgress / DECELERATE_DURATION_PERCENTAGE) * DECELERATE_DURATION_PERCENTAGE;
        } else {
            mAnimFrameValue = LINEAR_INTERPOLATOR.getInterpolation((renderProgress - DECELERATE_DURATION_PERCENTAGE) / ACCELERATE_DURATION_PERCENTAGE) * ACCELERATE_DURATION_PERCENTAGE + DECELERATE_DURATION_PERCENTAGE;
        }
    }
    ...
    private PointF getPoint1(RectF leafFlyRect) {
        ...
        int x = (int) (leafFlyRect.width() / 2), y = (int) leafFlyRect.height();
        if (x > 0 && y > 0) {
            point.x = leafFlyRect.right - mRandom.nextInt(x);
            point.y = (int) (leafFlyRect.bottom - mRandom.nextInt(y));
        }
