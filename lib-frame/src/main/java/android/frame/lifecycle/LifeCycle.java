...
import android.frame.Component;
...
public class LifeCycle {
     ...
     */
    private static final Handler mLifecycleHandler = new Handler(Looper.getMainLooper(), LifeCycle.mHandlerCallback);
    ...
    private static LifeCycleManager find(Context context) {
        ...
        if (context instanceof Activity) {
            lifeCycleManager = find((Activity) context);
        }
    ...
    @UiThread
    private static LifeCycleManager find(Activity activity) {
        LifeCycleManager lifeCycleManager = null;
    ...
    public static void notifyMainActivityFinish() {
        Loople.Impl.post(new Runnable() {
            ...
            @Override
            public void run() {
                FrameActivity activity = comp().main();
                if (!isFinishing(activity)) {
                    activity.finish();
                }

                comp().set((FrameActivity) null);
            }
        });
    }

    public static void reboot() {
        Intent intent = comp().app().getPackageManager().getLaunchIntentForPackage(getPackageName());
        intent.addFlags(FLAG_ACTIVITY_CLEAR_TOP | FLAG_ACTIVITY_NEW_TASK);

        ContextCompat.startActivity(comp().app(), intent, null);
    }

    public static void exit() {
        Loople.Impl.postDelayed(new Runnable() {
